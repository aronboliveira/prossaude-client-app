...
// const prevPosition: { [k: string]: number } = {};

// export function addButtonClickAnimationListeners(): void {
//   try {
//     document.querySelectorAll("button").forEach(btn => {
//       if (!(btn.id === "")) prevPosition[`${btn.id}`] = btn.clientTop;
//       btn.addEventListener("mousedown", ev => {
//         console.log("mousedown...");
//         try {
//           const transform = getComputedStyle(btn).transform;
//           console.log(transform);
//           if (transform === "") btn.style.transform = `translate(0, 0.1rem)`;
//           else {
//             if (!/translate/g.test(transform))
//               btn.style.transform = `${transform} translate(0, 0.1rem)`;
//             else {
//               const match = transform.match(
//                 /translate(Y)?\(\s*([0-9]+(?:px|rem|vw|vh|ch))\s*(?:,\s*([0-9]+(?:px|rem|vw|vh|ch))\s*)\)/g
//               );
//               console.log("MATCHES");
//               console.log(match ?? "none");
//               if (!match) return;
//               const measureMatch = match[0].match(/px|rem|vw|vh|ch/g);
//               if (!measureMatch?.at(-1)) return;
//               const lastMeasure = measureMatch.at(-1)!;
//               console.log("Last measure " + lastMeasure);
//               let inPxMeasure = 0;
//               if (/px/g.test(lastMeasure)) inPxMeasure = 2;
//               else if (/rem/g.test(lastMeasure)) inPxMeasure = 0.125;
//               else if (/vw/g.test(lastMeasure)) inPxMeasure = innerWidth / 50;
//               else if (/vh/g.test(lastMeasure)) inPxMeasure = innerHeight / 50;
//               else if (/ch/g.test(lastMeasure)) return;
//               const beforeTranslateY = transform
//                 .slice(
//                   transform.indexOf("translate"),
//                   match[0].replace(/\s*\)/, ")").indexOf(")") -
//                     lastMeasure.length
//                 )
//                 .trim();
//               console.log("before Y " + beforeTranslateY);
...
//               let addedTranslateY =
//                 parseNotNaN(translateY.replaceAll(/[^0-9]/g, "")) + inPxMeasure;
//               if (
//                 !Number.isFinite(addedTranslateY) ||
//                 !addedTranslateY ||
//                 addedTranslateY === 0
//               )
//                 addedTranslateY = 2;
//               btn.style.transform = `${beforeTranslateY} ${addedTranslateY}px)`;
//             }
//           }
//         } catch (e) {
//           console.error(
//             `Error applying ${ev.type} callback to ${
//               btn.id || btn.className || btn.tagName
//             }:\n${(e as Error).message}`
//           );
//         }
//         // setTimeout(() => {
//         //   if (prevPosition[`${btn.id}`] && btn.clientTop !== prevPosition[`${btn.id}`]) btn.clientTop =
//         // }, 2000);
//       });
...
//         try {
//           const transform = getComputedStyle(btn).transform;
//           if (transform === "") btn.style.transform = `translate(0, 0)`;
//           else {
//             if (!/translate/g.test(transform))
//               btn.style.transform = `${transform} translate(0, 0)`;
//             else {
//               const match = transform.match(
//                 /translate(Y)?\(\s*([0-9]+(?:px|rem|vw|vh|ch))\s*(?:,\s*([0-9]+(?:px|rem|vw|vh|ch))\s*)\)/g
//               );
...
//                 .trim();
//               console.log("translateY " + translateY);
//               let subtractedTranslateY =
//                 parseNotNaN(translateY.replaceAll(/[^0-9]/g, "")) - inPxMeasure;
//               if (
//                 !Number.isFinite(subtractedTranslateY) ||
//                 !subtractedTranslateY ||
//                 subtractedTranslateY === 0
//               )
//                 subtractedTranslateY = 2;
//               btn.style.transform = `${beforeTranslateY} ${subtractedTranslateY}px)`;
//             }
//           }
//         } catch (e) {
//           console.error(
...
//   } catch (e) {
//     console.error(
//       `Error executing adjustment for button clicking transformation:\n${
//         (e as Error).message
//       }`
//     );
//   }
