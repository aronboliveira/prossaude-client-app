...
describe("Watcher Component", (): void => {
  test("initially hides the watcher element", (): void => {
    ...
    (expect(screen.getByRole<HTMLElement>("group")) as jest.JestMatchers<jest.SpyInstance>).toHaveStyle(
      "display: none"
    ) as void;
  }) as void;
  test("calls modelScripts when handled", (): void => {
    render(<Watcher routeCase='login' />) as RenderResult;
    (expect(modelScripts) as jest.JestMatchers<jest.SpyInstance>).toHaveBeenCalled() as void;
  }) as void;
}) as void;
