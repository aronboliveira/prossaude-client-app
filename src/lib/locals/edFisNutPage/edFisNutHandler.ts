    ...
    if (
      ...
      (/Cálculo Automático/gi.test(autoFillBtn.innerText) ||
        (autoFillBtn.nextElementSibling instanceof HTMLElement &&
          /Cálculo Automático/gi.test(
            autoFillBtn.nextElementSibling.innerText
          )) ||
        (autoFillBtn.previousElementSibling instanceof HTMLElement &&
          /Cálculo Automático/gi.test(
            autoFillBtn.previousElementSibling.innerText
          )))
    )
...
export function switchLockInputs(
  locksTabInd: targEl[],
  autoFillActivation: boolean = false
): void {
  if (
    locksTabInd?.length > 0 &&
    ...
    locksTabInd.forEach(lock => {
      const siblingInput =
        lock?.parentElement?.parentElement?.querySelector(".tabInpProg");
      if (
...
export function validateEvResultNum(
  evEl: targEl,
  property: primitiveType = 0
): number {
  if (
    ((evEl instanceof HTMLInputElement &&
...
export function matchPersonPropertiesWH(
  ...
  if (person && Object.keys(person)?.length > 0) {
    ...
    objectError(
      "arguments de matchPersonPropertiesWH",
      person,
      "person",
      6,
      extLine(new Error())
...
export function matchPersonPropertiesDC(
  ...
  if (person && Object.keys(person)?.length > 0) {
    ...
    objectError(
      "arguments for matchPersonPropertiesDC",
      person,
      "person",
      6,
      extLine(new Error())
...
export function updateIndexesContexts(
  ...
  if (
    ...
    if (
      ...
      formatValue(targInpMLG, MLG);
    TMB =
      updateTMBContext(
        person,
        [targInpTMB, formTMBTypeElement],
        [...IMCArray, MLG] ?? [
          (gordCorpLvl as HTMLInputElement | HTMLSelectElement)?.value || 0,
          0,
          0,
        ],
        factorAtleta
...
export function updateIMCContext(
  ...
  if (
    ...
    if (
      ...
      typeError("gordCorpVal", gordCorpVal, "string", extLine(new Error()));
...
export function fluxFormIMC(
  ...
  if (
    ...
    if (
      (document.getElementById("nafType") as entryEl)?.value === "muitoIntenso"
    ) {
      if (!(formTMBTypeElement.value === "tinsley"))
        highlightChange(formTMBTypeElement);
      ...
      if (IMC >= 0 && IMC < 25.0) {
        ...
        console.error(`Error obtaining IMC value in fluxFormIMC(), line ${extLine(
          new Error()
        )}.
...
export function updateGETContext(
  ...
  const GET =
    parseNotNaN(
      person.calcGET(TMB || 0, factorAtvLvl).toFixed(4),
      0,
      "float"
    ) || 0;
  ...
  targInpGET instanceof HTMLSelectElement
    ...
    : inputNotFound(
        targInpGET,
        "targInpGET em updateGETContext",
        extLine(new Error())
      );
  ...
  if (
    ...
    if (/LvlAtFis/gi.test(mainSelect.id)) {
      ...
      if (
        ...
        inputNotFound(
          LvlAtFis,
          "LvlAtFis in matchTMBElements()",
          extLine(new Error())
        );
    } else
      stringError(
        "testing mainSelect.id in matchTMBElements()",
        mainSelect?.id,
        extLine(new Error())
      );
    ...
    if (mainSelect.value === "muitoIntenso") {
      ...
      if (
        ...
        console.error(`Error obtaining the value for Gordura Corporal, line ${extLine(
          new Error()
        )}.
          Obtained level of Gordura Corporal: ${gordCorpLvl?.value};
          Obtained IMC: ${IMC ?? 0}.`);
    } else
      console.error(`Error obtaining the value for mainSelect, line ${extLine(
        new Error()
      )}.
...
export function updatePGC(
  ...
  if (
    ...
    switch (context) {
      case "cons":
        targInpPGC = parentEl.querySelector(
          `#inpPgc${numRef}Cel4_${numRef + 1}`
        );
        targInpSumDCut = parentEl.querySelector(
    ...
    if (
      ...
      PGCDecayArray[0] === true
        ? formatValue(targInpPGC, PGC)
        : formatValue(targInpPGC, PGC, 2);
...
export function updateAtvLvl(
  ...
  if (
    ...
    if (typeof returnedAtvLvl === "string") {
      ...
      typeError(
        "update for mainSelect in updateAtLvl()",
        returnedAtvLvl,
        "string",
        extLine(new Error())
      );
  ...
  if (
    ...
    if (typeof numRef === "string") {
      ...
      numRef && numRef !== ""
        ...
        : stringError(
            "convertendo Número de Consulta de string for número",
            numRef,
            extLine(new Error())
          );
    }
    if (
      ...
      switch (context) {
        case "cons":
          arrayTargInps.push(
            parentEl.querySelector(`#tabInpRowMedAnt2_${numRef + 1}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#tabInpRowMedAnt3_${numRef + 1}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpImc${numRef}Cel2_${numRef + 1}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpMlg${numRef}Cel3_${numRef + 1}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpTmb${numRef}Cel5_${numRef + 1}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpGet${numRef}Cel6_${numRef + 1}`) ?? null
          );
          break;
        case "col":
          arrayTargInps.push(
            parentEl.querySelector(`#tabInpRowMedAnt2_${numRef}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#tabInpRowMedAnt3_${numRef}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpImc${numRef - 1}Cel2_${numRef}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpMlg${numRef - 1}Cel3_${numRef}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpTmb${numRef - 1}Cel5_${numRef}`) ?? null
          );
          arrayTargInps.push(
            parentEl.querySelector(`#inpGet${numRef - 1}Cel6_${numRef}`) ?? null
          );
          break;
      }
    } else
    ...
    if (arrayTargInps?.length === 6) {
      ...
      elementNotPopulated(arrayTargInps, "arrayTargInps", extLine(new Error()));
...
export function addRowAtivFis(
  ...
  if (typeof context === "string" && tBodyContainer) {
    ...
    if (document.querySelector(`tabRowAtFis${context}Id${count}`)) {
      syncAriaStates([
        ...document
          .querySelector(`tabRowAtFis${context}Id${count}`)!
          .querySelectorAll("*"),
  ...
  if (
    ...
    if (typeof numCons === "number" && numCons > 0 && numCons <= 3) {
      ...
      totalRows?.length > 0
        ...
        : elementNotPopulated(
            totalRows,
            "NodeList of <tr> in switchRequiredCols()",
            extLine(new Error())
          );
      ...
      totalCols?.length > 0
        ...
        : elementNotPopulated(
            totalCols,
            "NodeList of <col> in switchRequiredCols()",
            extLine(new Error())
          );
      ...
      if (
        ...
        Equals to the desired number for filling the axes: ${
          inpsCells.length === nTotalMatrixValidAxes ?? false
        };
        Accepted number: ${nTotalMatrixValidAxes / totalTables.length};
        Obtained number of <input> Elements for Sinais Vitais: ${
          inpsCellsSVi?.length ?? 0
        };
        Obtained number of <input> Elements for Medidas Antropométricas: ${
          inpsCellsMedAnt?.length ?? 0
        };
        Obtained number of <input> Elements for Dobras Cutâneas: ${
          inpsCellsDC?.length ?? 0
        };
        Obtained numbe of <input> Elements for Índices e Percentuais: ${
      ...
      let consRequiredCellsSVi: Element[][] = [],
      ...
      if (
        ...
        for (let iC = 0; iC < numCons; iC++) {
          ...
          consRequiredCellsSVi.push(
            ...(filterCellsPattern(inpsCellsSVi, filterPattern, iC) ?? [])
          );
          consRequiredCellsMedAnt.push(
            ...(filterCellsPattern(inpsCellsMedAnt, filterPattern, iC) ?? [])
          );
          consRequiredCellsDC.push(
            ...(filterCellsPattern(inpsCellsDC, filterPattern, iC) ?? [])
          );
          consRequiredCellsIndPerc.push(
            ...(filterCellsPattern(
              inpsCellsIndPerc,
              filterPattern,
              iC,
        ...
        Obtained validation array for NodeLists: ${
          validInpsNodeLists.toString() || null
        }`);
...
export function defineMatrixAxes(tab: targEl): number {
  ...
  if (tab instanceof HTMLTableElement) {
    ...
    elementNotFound(tab, "tab in defineMatrixAxes()", extLine(new Error()));
  return matrixValidAxes || 0;
}

export function validateTabInpList(
  inpsNL: NodeListOf<Element> | Array<Element>,
  nMatrix: number = 4
): boolean {
...
export function filterCellsPattern(
  ...
  if (
    ...
    switch (testAtrib) {
      case "id":
        filterInpCell = Array.from(inpCells).filter(inpCell =>
          filterPattern.test(inpCell.id)
        );
        break;
      case "name":
        filterInpCell = Array.from(inpCells).filter(inpCell =>
          filterPattern.test((inpCell as HTMLInputElement).name)
        );
        break;
      default:
        stringError(
          "argument for testAtrib in filterCellsPatern()",
          testAtrib,
          extLine(new Error())
        );
    }
    if (filterInpCell?.length > 0) {
      ...
      console.warn(
        `Error filtering .id of Elements in the table for Sinais Vitais, column ${columnNum}.`
      );
  } else
    multipleElementsNotFound(
      extLine(new Error()),
...
export function switchNumConsTitles(
  ...
  if (
    Array.from(consTitles)?.every(
      consTitle => consTitle instanceof HTMLElement
    ) &&
    ...
    Array.from(
      ...
      .forEach(input => {
        if (
          ...
          inputNotFound(input, `input id ${input?.id}`, extLine(new Error()));
      });
  } else
    multipleElementsNotFound(
      extLine(new Error()),
...
export function createArraysRels(
  ...
  if (
    ...
    const btnCol = parseNotNaN(
      btnId?.match(/(?<=_)[0-9]+/)?.toString() ?? "0",
      1
    );
    const targColInps = arrayRows.map(
      row =>
        Array.from(row.querySelectorAll("input")).filter(
          inp => inp.id?.match(`_${btnCol.toString()}`) ?? false
        )[0]
    );
...
export function getConsultasNums(arrayRow: targEl): number[] {
  ...
  if (arrayRow instanceof HTMLTableRowElement) {
    ...
    for (let iL = 0; iL < strConsultasNum.length; iL++) {
      arrayConsultasNum = arrayConsultasNum.concat(
        parseNotNaN(strConsultasNum?.slice(0 + iL, 1 + iL) ?? "0", 1)
      );
    }
  } else
    elementNotFound(
      arrayRow,
      "arrayRow in getConsultasNum",
      extLine(new Error())
    );
