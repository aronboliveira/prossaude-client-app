...
export function addListenerInspRadios(): Element[] {
  const inspRadios = [
    ...document.querySelectorAll(".radYes"),
    ...document.querySelectorAll(".radNo"),
  ];
...
export function addListenerInspDialogBtns(
  ...
  if (quadrsTe?.length > 0) {
    quadrsTe.forEach(quadrTo => {
      if (quadrTo instanceof HTMLElement) {
        quadrTo.addEventListener("mousemove", () =>
          OdHandler.dragHover(quadrTo)
        );
        ...
        elementNotFound(
          quadrTo,
          `${quadrTo?.id ?? "UNDEFINED QUADRANT ID"}`,
          extLine(new Error())
        );
    });
  } else
    elementNotPopulated(
      quadrsTe,
      "quadrsDents in odController",
      extLine(new Error())
    );
  return quadrsTe;
}

export function addListenerAvElemenDents(
  isValuePreDef: boolean = false
): [boolean, Element[]] {
  const avElemDentsArray = Array.from(
    ...
    if (subDivsQuadrs?.length > 0) {
      subDivsQuadrs.forEach(subDiv => {
        subDiv instanceof HTMLElement
          ...
          : elementNotFound(
              subDiv,
              "subDiv in odController",
              extLine(new Error())
            );
      });
    } else
      elementNotPopulated(subDivsQuadrs, "subDivsQuadrs", extLine(new Error()));
  });
  return Array.from(subDivsQuadrs);
}

export function addListenerTratContainer(
  blockCount: number = 1
): [number, Element[]] {
