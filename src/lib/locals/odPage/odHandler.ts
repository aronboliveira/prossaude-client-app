...
export function dragDrop(
  ...
  while (
    ...
    if (
      validTargEl instanceof HTMLDivElement &&
      validTargEl.classList.contains("quadrAvDent")
    )
      break;
  }
...
export function dragEnd(movingSrcEl: targEl): void {
  ...
  movingSrcEl instanceof HTMLElement
    ...
    : elementNotFound(
        movingSrcEl,
        "movingSrcEl in dragEnd",
        extLine(new Error())
      );
}

export function dragStartChilds(
  contInQuadrs: Element[] | NodeListOf<Element>
): void {
...
export function clearQuadrInps(quadrInp: targEl): void {
  if (
    ...
    if (quadrInp.nextElementSibling) {
      ...
      elementNotFound(
        quadrInp.nextElementSibling,
        "quadrInp.nextElementSibling",
        extLine(new Error())
      );
  } else
    inputNotFound(quadrInp, "quadrInp in clearQuadrInps", extLine(new Error()));
}
...
export function resetLabels(quadrBtn: targEl): void {
  ...
  if (
    quadrBtn instanceof HTMLButtonElement &&
    parentDiv &&
    innerDivInps &&
    innerDivInps?.length > 0
  ) {
    if (innerDivInps.length < 8)
      console.warn(
        `Error validating inputs in the quadrant. Total number of inputs obtained: ${innerDivInps.length}`
      );
    else
...
export function addSubDivTrat(
  click: Event | React.MouseEvent,
  addSubDivBtn: targEl,
  blockCount: number = 1
): number {
  if (
    click.currentTarget === addSubDivBtn &&
    addSubDivBtn instanceof HTMLButtonElement
  ) {
    if (addSubDivBtn.classList.contains("addTrat")) {
      ...
      if (document.querySelector(`divSubTrat${blockCount}`)) {
        syncAriaStates([
          ...document
            .querySelector(`divSubTrat${blockCount}`)!
            .querySelectorAll("*"),
      ...
      const divToRemove = Array.from(
        document.querySelectorAll(".divSubTrat")
      ).at(-1);
