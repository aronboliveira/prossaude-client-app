...
export default function EraseAptBtn({
  userClass = "estudante",
}: GlobalFormProps) {
    ...
    try {
      if (!(btnRef.current instanceof HTMLButtonElement))
        throw elementNotFound(
          btnRef.current,
          `${(btnRef.current as any)?.id ?? "UNIDENTIFIED"}`,
          extLine(new Error())
        );
      ...
      if (!(relSlot instanceof HTMLElement))
        throw elementNotFound(
          relSlot,
          "slot for erase appointment btn",
          extLine(new Error())
        );
      btnRef.current.id = btnRef.current.id.replace(
        "unfilled",
        relSlot.id.replace("slot_", "")
      );
    } catch (err) {
  ...
  return (
    <ErrorBoundary
      FallbackComponent={() => (
        <GenericErrorComponent message="Erro carregando botão de excluir" />
      )}
    >
      <button
        type="button"
        ...
        onClick={() => {
          (userClass === "coordenador" || userClass === "supervisor") &&
            setDisplayExcludeDlg(!shouldShowExcludeDlg);
        }}
      ></button>
