...
export default function AptDataList({
  ...
  useEffect(() => {
    if (aptDlgRef.current instanceof HTMLDialogElement) {
      ...
      aptDlgRef.current.addEventListener(
        "click",
        function (this: HTMLDialogElement, click) {
      ...
      syncAriaStates([
        ...aptDlgRef.current!.querySelectorAll("*"),
        aptDlgRef.current,
      ]);
      ...
      btnExport instanceof HTMLButtonElement
        ? addListenerExportBtn(
            ...
            aptDlgRef.current,
            (aptDlgRef.current.querySelector(
              `[id*="outpNamePac"]`
            ) as HTMLOutputElement) ?? (aptDlgRef.current as HTMLElement)
          )
        : elementNotFound(
            btnExport,
            "Button for generating spreadsheet in appointment modal",
            extLine(new Error())
          );
      transferBtn instanceof HTMLButtonElement
        ? transferBtn.addEventListener("click", () => {
            ...
            shouldDisplayAptList
              ? aptDlgRef.current?.showModal()
              : aptDlgRef.current?.close();
          })
        : elementNotFound(
            transferBtn,
            "transferBtn for AptDataList",
            extLine(new Error())
          );
      ...
      {shouldDisplayAptList && (
        ...
        >
          ...
          >
            <div
              role="group"
              className="flexRNoWBetCt cGap2v widQ460_120v"
              id="headRegstPac"
            >
              <h2 className="mg-1b">
              ...
              <tbody>
                {userClass === "coordenador" && (
                  <tr className="rowCPFPacCons">
                    ...
                    <td>
                      <output
                        id={`outpCPFPac${data.cpf || "Unidentified"}_${
                          data.date || "0000-00-00"
                        }`}
                        name={`CPFPac${data.cpf || "Unidentified"}_${
                          data.date || "0000-00-00"
                        }-out`}
                ...
                <tr className="rowNamePacCons">
                  ...
                  <td>
                    <output
                      id={`outpNamePac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }`}
                      name={`namePac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      data-title={`Nome Paciente ${data.name || "Anônimo"}_${
                        data.date || "0000-00-00"
                      }-out`}
                ...
                <tr className="rowTelPacCons">
                  ...
                  <td>
                    <output
                      id={`outpTelPac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }`}
                      name={`namePac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                ...
                <tr className="rowEmailPacCons">
                  ...
                  <td>
                    <output
                      id={`outpEmailPac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }`}
                      name={`emailPac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      data-title={`Nome Paciente ${data.name || "Anônimo"}_${
                        data.date || "0000-00-00"
                ...
                <tr className="rowTypeCons">
                  ...
                  <td>
                    <output
                      id={`outpTypeConsPac${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }`}
                      name={`typeCons${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      data-title={`Tipo da Consulta Paciente ${
                        data.name || "Anônimo"
                      }_${data.date || "0000-00-00"}-out`}
                    >{`${
                      data.typecons?.slice(0, 1).toUpperCase() +
                        data.typecons?.slice(1, data.typecons.length) ||
                      "Indefinida"
                    }`}</output>
                  </td>
                </tr>
                <tr className="rowStudCons">
                  ...
                  <td>
                    <output
                      id={`outpStudCons${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      name={`relStud${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      data-title={`Nome do Estudante Alocado Paciente ${
                        data.name || "Anônimo"
                      }_${data.date || "0000-00-00"}-out`}
                ...
                <tr className="rowProfCons">
                  ...
                  <td>
                    <output
                      id={`outpProfCons${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      name={`relProf${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }`}
                      data-title={`Nome do Profissional Alocado Paciente ${
                        data.name || "Anônimo"
                      }_${data.date || "0000-00-00"}-out`}
                ...
                <tr className="rowNotes">
                  ...
                  <td>
                    <output
                      id={`outpNotesCons${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      name={`notes${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      data-title={`Notas Paciente ${data.name || "Anônimo"}_${
                        data.date || "0000-00-00"
                      }-out`}
                ...
                <tr className="confirmCons">
                  ...
                  <td>
                    <output
                      id={`outpConfirmCons${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      name={`confirm${data.cpf || "Unidentified"}_${
                        data.date || "0000-00-00"
                      }-out`}
                      data-title={`Confirmação Paciente ${
                        data.name || "Anônimo"
                      }_${data.date || "0000-00-00"}-out`}
            ...
            <div
              role="group"
              className="flexWR flexAlItCt flexJSb widFull widQ460MinFull"
              id="divExportPac"
            >
              <button
                type="button"
                id={`btnExport${data.cpf || "Unidentified"}_${
                  data.date || "0000-00-00"
                }}`}
                name={`btnExport${data.cpf || "Unidentified"}_${
                  data.date || "0000-00-00"
                }`}
