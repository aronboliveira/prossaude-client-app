...
export default function UserDlg({
  ...
  //TODO REPLICA DO USERDROPDOWN PARA CASOS DE ERRO EM STACKING CONTEXT
  ...
  useEffect(() => {
    if (userDlgRef.current instanceof HTMLDialogElement) {
      ...
      syncAriaStates([
        ...userDlgRef.current.querySelectorAll("*"),
        userDlgRef.current,
      ]);
    } else
      elementNotFound(
        userDlgRef.current,
        "Dialog for user panel",
        extLine(new Error())
      );
  }, [userDlgRef]);
  return (
    <ErrorBoundary
      FallbackComponent={() => (
        <GenericErrorComponent message="Erro carregando janela modal" />
      )}
    >
      <dialog
        className="bdPo noInvert modal-content-fit forceInvert"
        ref={userDlgRef}
      >
        <ErrorBoundary
          FallbackComponent={() => (
        ...
        >
          ...
          <dl className="mg-0b">
            ...
            <dd id="classLoginDlg" data-title="Classe de Usuário ativo">
              {`${user.userClass
                .slice(0, 1)
                .toUpperCase()}${user.userClass.slice(1)}
    `}
            </dd>
            <dt>Área:</dt>
            <dd>{`${user.userArea
              .slice(0, 1)
              .toUpperCase()}${user.userArea.slice(1)}`}</dd>
            <dt>E-mail:</dt>
