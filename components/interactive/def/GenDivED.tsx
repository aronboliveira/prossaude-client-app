...
import { AlignType, BirthRelation, Gender, GordLvl, TransitionLevel } from "@/lib/tests/testVars";
import { ENContext } from "../edfis/ENForm";
import { ENContextProps } from "@/lib/global/declarations/interfaces";
import { evalGender } from "@/lib/locals/edFisNutPage/edFisNutModel";
import { GenDivProps } from "@/lib/global/declarations/interfacesCons";
import useGenDiv from "@/lib/hooks/useGenDiv";
export default function GenDivED({ onSetGen, genRef, genBirthRef }: GenDivProps): JSX.Element {
  const {
  ...
  const {
      gen: ctxGen,
      refs: { gar: ctxGar },
    } = useContext<ENContextProps>(ENContext),
    tbr = useRef<nlSel>(null);
  useEffect(() => {
    if (evalGender(ctxGen)) setGen(ctxGen);
  }, [setGen]);
  useEffect(() => {
    if (ctxGar) ctxGar.current = gar.current;
  }, []);
  useEffect(() => {
    try {
              ...
              try {
                ...
                if ((gt.value !== "avancado" || selectedGen === "naoBinario") && !gt.hidden && !ga.hidden) {
                  if (tbt) {
                    ...
                    setTextBodytype(tbt.value as GordLvl);
