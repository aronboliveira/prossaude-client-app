...
export default function WatcherEN(): JSX.Element {
  ...
  useEffect(() => {
    ...
    tabProps.edGenValue =
      ...
      genElement instanceof HTMLTextAreaElement
        ? genElement.value
        : "masculino";
    tabProps.edIsAutoCorrectOn = getGlobalEls(
  ...
  useEffect(() => {
    if (mounted && document.getElementsByTagName("table").length > 3) {
      ...
      try {
        ...
        tabProps.numCons =
          parseNotNaN((selectNumCons as entryEl)?.value || "1") || 1;
        ...
        tabProps.numConsLastOp = parseNotNaN(
        ...
        if (
          !(
            tabProps.numConsLastOp === tabProps.numColsCons &&
            tabProps.numConsLastOp >= 3
          )
        ) {
          console.error(`
        ...
        parseNotNaN(
          (
            document.getElementById(
              `tabInpRowDCut9_${tabProps.numCons + 1}`
            ) as entryEl
          )?.value ?? "0",
          0,
          "float"
      ...
      person.weight =
        parseNotNaN(
          (
            document.getElementById(
              `tabInpRowMedAnt2_${tabProps.numCons + 1}`
            ) as entryEl
          )?.value ?? "0",
          0,
          "float"
      ...
      person.height =
        parseNotNaN(
          (
            document.getElementById(
              `tabInpRowMedAnt3_${tabProps.numCons + 1}`
            ) as entryEl
          )?.value ?? "0",
          0,
          "float"
      ...
      [tabProps.numColsCons, tabProps.areColGroupsSimilar] =
        addListenerInnerTabs(
          consTablesFs,
  ...
  return (
    <div className="watcher" id="watcher-en" style={{ display: "none" }}></div>
  );
}
