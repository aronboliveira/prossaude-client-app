...
export default function TrioReadNumCons(): JSX.Element {
  ...
  return (
    <label htmlFor='trioReadNumCons' id='labTrioReadNumCons' className='consLab' ref={mainRef}>
      ...
      <input
        ...
        onInput={ev => {
          try {
            ...
            const numTotalTabsCons = document.getElementById("fsProgConsId")?.querySelectorAll("table")?.length || 0;
            if (
            ...
            let numTotalTitledColsCons = document.getElementById("fsProgConsId")?.querySelectorAll("col")?.length || 0;
            const numConsTextHeadCels = Array.from(document.getElementsByClassName("numConsTextHeadCel"));
            const tabsNum = document.getElementById("fsProgConsId")?.querySelectorAll("table")?.length || 0;
            numTotalTitledColsCons = numTotalTitledColsCons - tabsNum;
            numConsTextHeadCels.length === numTotalTitledColsCons
              ? switchNumConsTitles(numConsTextHeadCels, ev.currentTarget, numTotalTitledColsCons, numTotalTabsCons)
              : elementNotPopulated(
                  numConsTextHeadCels,
            ...
            console.error(
              `Error executing callback for ${ev.currentTarget.id || ev.currentTarget.tagName}:\n${
                (e as Error).message
              }`,
            );
          }
