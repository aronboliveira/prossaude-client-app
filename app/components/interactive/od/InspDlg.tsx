...
export default function InspDlg({ count, ctx, state, dispatch }: InspDlgProps): JSX.Element {
  ...
  return (
    <ErrorBoundary FallbackComponent={() => <GenericErrorComponent message='Error loading Inspection Dialog' />}>
      {state && (
        <dialog className='inspDialog modal-content' id={`inspDialog${count}`} draggable='true' ref={dlgRef}>
          <ol className='inspList' id={`inspList${count}`}>
            <span role='group' className='inspLITitle modal-title noInvert'>
              ...
              <button className='btn btn-close' onClick={() => dispatch(!state)}></button>
            </span>
            <div role='group' className='modal-body'>
              {(ctx === "lab" || ctx === "jug" || ctx === "vest" || ctx === "pltm" || ctx === "of" || ctx === "lg") && (
                <>
              ...
              {(ctx === "lab" || ctx === "jug" || ctx === "pltm" || ctx === "lg") && (
                <>
